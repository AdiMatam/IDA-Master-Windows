...
linkmap.update({
    ...
    "klu": linkmap["standard"] + 
        Spec(
            compiledirs = ["./suitesparse/include/suitesparse"],
            linkdirs = ["./suitesparse/lib"],
            links = ["sundials_sunlinsolklu", "klu", "amd", "colamd", "btf", "suitesparseconfig", "gomp"],
    )
...
def main():
    ...
    # python3 py_runner.py sundials/examples/ida/serial/idaHeat2D_sps superlu
    sample = sys.argv[1]
    ptype = sys.argv[2]
    ...
    sub.run(compile_call, shell=True, check=True)
    sub.run(link_call, shell=True, check=True)

    if len(sys.argv) > 3:
        if sys.argv[3] == "run":
            sub.run(f"./{exe}.exe {''.join(sys.argv[4:])}", shell=True, check=True)
